...
from train_classifier import SCAM_LABELS, LABEL2ID, make_dataset, load_jsonl
...
def evaluate(args):
    ...
    header = f"{'':20s}" + "".join(f"{l[:8]:>10}" for l in SCAM_LABELS)
    print(header)
    for i, row in enumerate(cm):
        print(f"{SCAM_LABELS[i]:20s}" + "".join(f"{v:>10}" for v in row))
